...
- Source: Organization Manager.
- Stimulus: Enters counts for up to 50 Consumers and 10 Products.
...
- Source: Organization Manager.
- Stimulus: Previews a Tally containing up to 50 Consumers and 10 active Products with relative
  width weights.
- Environment: A3 landscape output.
- Response: PROST wraps Product headers, repeats headings on vertical pages, keeps all Product
  columns together, and exposes an illegible configuration before issuance.
- Measure: Issuance is blocked if the accepted minimum marking-width policy cannot be met. This
  scenario is not acceptance-ready until `OQ-008` defines that policy.
